...
class CardOffer(models.Model): 
    ...
    auction_current_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
...
class ShippingData(models.Model):
    ...
    card_offer = models.ForeignKey(CardOffer, on_delete=models.CASCADE, related_name='shipping')
    city = models.CharField(max_length=100)
    zip_code = models.CharField(max_length=10)
    street = models.CharField(max_length=150)
    house_number = models.CharField(max_length=10)
    ...
    def __str__(self):
        return f"{self.card_offer} Delivery complited = {self.delivery_completed}"
